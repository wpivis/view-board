/*jshint esversion: 6 */
/*jslint node: true */
/*jshint strict:true */

'use strict';

var fs = require("fs");
var util = require('util');
var path = require('path');
var events = require("events");
var eventEmitter = events.EventEmitter();
var SerialPort = require('serialport');
var tinycolor = require('tinycolor2');

var letters = require("./letters.json");

const BOARD_SIZE = 7; //7x7 board

//Opcodes
const START_BIT = 128;
const END_BIT = 173;
const KEY = 19;
const SET = 104;
const UPDATE = 105;
const CLEAR = 106;
const SETM = 107;

/**
 * Handles generic errors
 * @param  {Error} 	err  Returning Error
 * @param  {string} type Type of Error
 * @return null
 */
function errorHandler(err, type) {
	if (err) {
		switch(type) {
			case 'W':
			console.log("nopixel: Error Writing: ", err);
			break;
			default:
			console.log ("nopixel: Error: ", err);
			break;
		}
	}
}

/**
 * Parses incoming packets
 * @param  {Buffer} data 	Buffer with incoming data
 * @return {Array}      	array of packets
 */
function parsePackets(data) {
	var packetArray = [];
	for (var i=0;i<data.length-1;i++) {
		try {
			// Check if we found our opcode. 
			// However, since past-me was a dum dum who thought 0x13 
			// would make a good opcode, even though it's blatantly
			// in range of the button index, now-me has the privilge
			// to implement this workaround, by checking if the 
			// status bit of the buffer is larger than 4, which is 
			// the maximum value of the bit. Good job, past-Nam.
			if (data[i] == KEY && (data[i+3] < 4)) {
				var packetLength = data[i+1] + 2;
				// console.log("Found a command, " + packetLength + " bits long");

				//Checksum
				var sum = 0;
				for(var j=i;j<i+packetLength-1;j++) {
					sum += data[j];
				}
			
				//If valid, add to array
				if (sum + data[i+packetLength-1] == 256) {
					// console.log("Valid packet, adding to packet array.");
					var tempBuffer = new Buffer(packetLength-3);
					// Strip header, n-bytes and checksum since we don't need them anymore
					data.copy(tempBuffer, 0, i+2, i+packetLength-1);
					// console.log(tempBuffer);
					packetArray.push(tempBuffer);
				} else {
					throw "Invalid/Corrupted packet";
				}
			}

		} catch(err) {
				errorHandler(err);
			}
	}

	//Array of packets, eachPl starts with a header and ends with checksum 
	return packetArray;
}

/**
 * Converts Index to XY Coordinate
 * @param  {Number} index 	Index number
 * @return {Object} 		XY Object
 */
function toCoordinate(index) {
	try {
		if(index >= (BOARD_SIZE*BOARD_SIZE) - 1 || index < 0) {
			throw "Index (coordinate) out of range.";
			return -1;
		}

		return {
			x: index%BOARD_SIZE,
			y: parseInt(index/BOARD_SIZE)
		};

	} catch(err) {
		errorHandler(err);
	}  

}

/**
 * Converts XY Coordinate to Index
 * @param  {Number} x X-coordinate
 * @param  {Number} y Y-coordinate
 * @return {Number}   Index number
 */
function toIndex(x, y) {
	try {
		if(x >= BOARD_SIZE || y >= BOARD_SIZE || x < 0 || y < 0) {
			throw "Index (x,y) out of range.";
			return -1;
		}

		return (y*BOARD_SIZE) + x;

	} catch(err) {
		errorHandler(err);
	}
}

var Nopixel =  function(configPath) {
	var self = this;
	self.configPath = configPath;
	events.EventEmitter.call(self);

	// Config with default values
	var systemConfiguration = JSON.parse(fs.readFileSync(configPath, "utf-8"));
	var comPort = systemConfiguration[0].comPort || "/dev/ttyACM0";
	var baudRate = systemConfiguration[0].baudRate || 9600;

	var port = new SerialPort(comPort, {baudrate: baudRate});
	console.log("nopixel: Successfully connected to device on port " + comPort + ".");
	
	// !!Hacky, wait for controller to initialize
	setTimeout(function() {
		self.emit("ready");	
	},1);

	port.on("error", (err) => errorHandler(err));
	port.on('data', function (data) {
		var packets = parsePackets(data);
		if (packets.length) {
			packets.forEach(function(el) {
				var eventDetail = {
					x: toCoordinate(el[0]).x,
					y: toCoordinate(el[0]).y,
					index: el[0]
				}
				switch(el[1]) {
					case 0: //Pressed
					self.emit("clicked", eventDetail);
					break;
					case 1: //Hold
					self.emit("pressed", eventDetail);
					break;
					case 2:
					self.emit("released", eventDetail);
					break;
					case 3:
					self.emit("idle", eventDetail);
				}
			})
		} 
		// else {
		// 	console.log("Got something: ");
		// 	console.log(data.toString('ascii'));
		// } 
	});

	/**
	 * Sets a specific pixel
	 * @param {Number} 			x           X-coordinate
	 * @param {Number} 			y           Y-coordinate
	 * @param {String|Object} 	colorString Color string/object - see tinycolor2 inputs
	 * @return {Object} 					Nopixel object
	 */
	self.setPixel = function(x, y, colorString) {
		var colorObj = tinycolor(colorString);
		var index = toIndex(x,y);

		try {
			if(index < 0) {
				throw "Index out of range.";
				return;
			}
			if (colorObj.isValid()) {
				var colorRGB = colorObj.toRgb();

				port.write(new Buffer.from([START_BIT,SET,toIndex(x,y),colorRGB.r,colorRGB.g,colorRGB.b,END_BIT]),
					(err) => errorHandler(err, 'W'));

			} else throw colorString + " is not a valid color.";
		} catch(err) {
			errorHandler(err);
		}

		return self;
	}

	/**
	 * Renders the LEDs
	 * @return {Object} Nopixel object
	 */
	self.update = function() {
		port.write(new Buffer.from([START_BIT, UPDATE, END_BIT]), (err) => errorHandler(err, 'W'));

		return self;
	}

	/**
	 * Clears the LEDs
	 * @return {Object} Nopixel object
	 */
	self.clear = function() {
		port.write(new Buffer.from([START_BIT, 106, END_BIT]), (err) => errorHandler(err, 'W'));

		return self;
	}

	/**
	 * Sets the whole board from node-canvas data
	 * @param  {TypedArray} data 	Canvas board data 
	 * @return {Object}      		Nopixel object
	 */
	self.fromCanvas = function(data) {
		//Clean up data by removing the alpha channel
		data = Array.from(data.filter(function(el,i) {return i%4 != 3}));
		data = [START_BIT, SETM, 0, 48].concat(data, [END_BIT]);

		port.write(new Buffer.from(data), (err) => errorHandler(err, 'W'));

		return self;
	}

	/**
	 * Display scrolling text
	 * @param  {String} 		string 			String to write
	 * @param  {color}  		ColorObject		A valid tinyColor2 object
	 * @param  {scrollSpeed}	Number 			Delay interval 
	 * @return {Object}      	Interval object
	 */
	self.write = function(string, color, scrollSpeed) {
		scrollSpeed = scrollSpeed || 1000;
		var buffer = new Array(7).fill(new Array(7).fill(0));
		var startx = 6;
		var starty = 1;
		var count = 0;

		var length = 0;
		for(var i = 0; i < string.length; i++) {
			var letter = letters[string[i]];
			length+= letter.length;
		}

		var writeInterval = setInterval(function() {
			self.clear();
			var letterOffset = 0;
			for(var i = 0; i < string.length; i++) {
				self.writeLetter(string[i], color, startx-count+letterOffset, starty);
				letterOffset+= letters[string[i]].length;
			}
			if(count > length) {
				count = 0;
			} else {
				count++;
			}
			self.update();
		}, scrollSpeed);

		return writeInterval;
	}

	/**
	 * Display a single letter at coordinate x-y
	 * @param  {String} 		string 		Character to display
	 * @param  {Color object}	color 		A valid TinyColor2 object
	 * @param  {Number}  		coordx 		Start coordinate x
	 * @param  {Number}			coordy		Start coordinate y
	 * @return {Object}      		Nopixel object
	 */
	self.writeLetter = function(string, color, coordx, coordy) {
		var letter = letters[string];
		for(var y = 0; y < letter.length; y++) {
			var row = letter[y];
			for(var x = 0; x < row.length; x++) {
				if(row[x] && (x+coordx) < 7 && (y+coordy) < 7 && (x+coordx) >= 0 && (y+coordy) >= 0) {
					self.setPixel(x+coordx, y+coordy, color);
				}
			}
		}

		return self;
	}

	return self;
}


Nopixel.prototype.__proto__ = events.EventEmitter.prototype;
module.exports = Nopixel;