...
var Nopixel =  function(configPath) {
	...
	self.setPixel = function(x, y, colorString) {
		...
		if (colorObj.isValid()) {
			...
			// console.log("Sending color", colorRGB);
			port.write(new Buffer.from([startBit,104,toIndex(x,y),colorRGB.r,colorRGB.g,colorRGB.b,endBit]),
	...
	self.update = function() {
		port.write(new Buffer.from([startBit, 105, endBit]), (err) => errorHandler(err, 'W'));
	}

	self.clear = function() {
		port.write(new Buffer.from([startBit, 106, endBit]), (err) => errorHandler(err, 'W'));
	}
	...
	self.fromCanvas = function(data) {
		...
		data = [startBit, 107, 0, 48].concat(data, [173]);

		console.log(data[data.length-1]);
		console.log(data.slice(0,4));
		for(var i=0;i<49;i++) {
			console.log(data[i*3+4], data[i*3+5], data[i*3+6])
		}
	...
	self.close = function() {
		port.flush((err) => console.log(err));
		port.close((err) => console.log(err));
	}
